...
abstract class BaseFriend extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the user1_id field.
	 * @var        int
	 */
	protected $user1_id;

	/**
	 * The value for the user2_id field.
	 * @var        int
	 */
	protected $user2_id;

	/**
	 * The value for the friend_group_id field.
	 * @var        int
	 */
	...
	/**
	 * Get the [friend_group_id] column value.
	 * 
	 * @return     int
	 */
	 ...
	 */
	public function getRequestedDate($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->requested_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->requested_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->requested_date, true), $x);
			}
		}
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getConfirmedDate($format = 'Y-m-d H:i:s')
	{
		if ($this->confirmed_date === null) {
			return null;
		}


		if ($this->confirmed_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->confirmed_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->confirmed_date, true), $x);
			}
		}
	...
	/**
	 * Set the value of [user1_id] column.
	 * 
	 * @param      int $v new value
	 * @return     Friend The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [user2_id] column.
	 * 
	 * @param      int $v new value
	 * @return     Friend The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [friend_group_id] column.
	 * 
	 * @param      int $v new value
	 * @return     Friend The current object (for fluent API support)
	 */
	public function setFriendGroupId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->friend_group_id !== $v) {
			$this->friend_group_id = $v;
			$this->modifiedColumns[] = FriendPeer::FRIEND_GROUP_ID;
		}
	...
	public function setRequestedDate($v)
	{
		...
		if ( $this->requested_date !== null || $dt !== null ) {
			...
			$currNorm = ($this->requested_date !== null && $tmpDt = new DateTime($this->requested_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->requested_date = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = FriendPeer::REQUESTED_DATE;
			}
	...
	public function setConfirmedDate($v)
	{
		...
		if ( $this->confirmed_date !== null || $dt !== null ) {
			...
			$currNorm = ($this->confirmed_date !== null && $tmpDt = new DateTime($this->confirmed_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->confirmed_date = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = FriendPeer::CONFIRMED_DATE;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->user1_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->user2_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->friend_group_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->requested_date = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->confirmed_date = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = FriendPeer::NUM_COLUMNS - FriendPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getUser1Id(),
			$keys[2] => $this->getUser2Id(),
			$keys[3] => $this->getFriendGroupId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setUser1Id($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUser2Id($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setFriendGroupId($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setRequestedDate($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setConfirmedDate($arr[$keys[5]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FriendPeer::USER1_ID)) $criteria->add(FriendPeer::USER1_ID, $this->user1_id);
		if ($this->isColumnModified(FriendPeer::USER2_ID)) $criteria->add(FriendPeer::USER2_ID, $this->user2_id);
		if ($this->isColumnModified(FriendPeer::FRIEND_GROUP_ID)) $criteria->add(FriendPeer::FRIEND_GROUP_ID, $this->friend_group_id);
		if ($this->isColumnModified(FriendPeer::REQUESTED_DATE)) $criteria->add(FriendPeer::REQUESTED_DATE, $this->requested_date);
		if ($this->isColumnModified(FriendPeer::CONFIRMED_DATE)) $criteria->add(FriendPeer::CONFIRMED_DATE, $this->confirmed_date);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setFriendGroupId($this->friend_group_id);
		$copyObj->setRequestedDate($this->requested_date);
		$copyObj->setConfirmedDate($this->confirmed_date);
