...
/**
 * Base class that represents a row from the 'video_file' table.
 *
 * 
 *
...
abstract class BaseVideoFile extends BaseObject  implements Persistent
{
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = VideoFilePeer::USER_ID;
		}
	...
	public function setArtist($v)
	{
		...
		if ($this->artist !== $v) {
			$this->artist = $v;
			$this->modifiedColumns[] = VideoFilePeer::ARTIST;
		}
	...
	public function setAlbum($v)
	{
		...
		if ($this->album !== $v) {
			$this->album = $v;
			$this->modifiedColumns[] = VideoFilePeer::ALBUM;
		}
	...
	public function setTrack($v)
	{
		...
		if ($this->track !== $v) {
			$this->track = $v;
			$this->modifiedColumns[] = VideoFilePeer::TRACK;
		}
	...
	public function setComments($v)
	{
		...
		if ($this->comments !== $v) {
			$this->comments = $v;
			$this->modifiedColumns[] = VideoFilePeer::COMMENTS;
		}
	...
	public function setPath($v)
	{
		...
		if ($this->path !== $v) {
			$this->path = $v;
			$this->modifiedColumns[] = VideoFilePeer::PATH;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 8; // 8 = VideoFilePeer::NUM_COLUMNS - VideoFilePeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(VideoFilePeer::ID) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(VideoFilePeer::USER_ID)) $criteria->add(VideoFilePeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(VideoFilePeer::ARTIST)) $criteria->add(VideoFilePeer::ARTIST, $this->artist);
		if ($this->isColumnModified(VideoFilePeer::ALBUM)) $criteria->add(VideoFilePeer::ALBUM, $this->album);
		if ($this->isColumnModified(VideoFilePeer::TITLE)) $criteria->add(VideoFilePeer::TITLE, $this->title);
		if ($this->isColumnModified(VideoFilePeer::TRACK)) $criteria->add(VideoFilePeer::TRACK, $this->track);
		if ($this->isColumnModified(VideoFilePeer::COMMENTS)) $criteria->add(VideoFilePeer::COMMENTS, $this->comments);
		if ($this->isColumnModified(VideoFilePeer::PATH)) $criteria->add(VideoFilePeer::PATH, $this->path);
